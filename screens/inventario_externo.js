...
const Inventario_ext = (props) => {

  const[products, setUsers] = useState([])

  useEffect(() => {
    firebaseService.database.collection('inventario_ext').onSnapshot(querySnapshot => {
      const products =[];
      querySnapshot.docs.forEach(doc => {
        const {nombre, codigo, cantidad, precio} = doc.data()
        products.push({
        id: doc.id,
        nombre,
        codigo,
    ...
    return(  
      <ScrollView >
          <TouchableOpacity style={styles.botonAltas} onPress={() => props.navigation.navigate('AltasExt')}>
          ...
          products.map((product) =>{
            return(
              <ListItem key={product.key} bottomDivider onPress={() => {
                props.navigation.navigate('DetalleExt' ,{
