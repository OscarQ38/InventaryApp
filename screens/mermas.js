...
const Mermas = (props) => {
  ...
  useEffect(() => {
    firebaseService.database.collection('Mermas').onSnapshot(querySnapshot => {
      const products =[];
      querySnapshot.docs.forEach(doc => {
        const {nombre, codigo, mermas} = doc.data()
        products.push({
        id: doc.id,
        nombre,
        codigo,
    ...
    return(  
      <ScrollView >
          <TouchableOpacity style={styles.botonAltas} onPress={() => props.navigation.navigate('AltasM')}>
          ...
          products.map((product) =>{
            return(
              <ListItem key={product.key} bottomDivider onPress={() => {
                props.navigation.navigate('DetalleM' ,{
            ...
            <ListItem.Content>
              ...
              <ListItem.Subtitle>{product.mermas}</ListItem.Subtitle>
            </ListItem.Content>
            </ListItem>
            )
          })
